...
class IndexUtil:
    ...
    # 创建向量索引 ,需要传入生成向量的原属性，以及嵌入属性
    def create_vector_index(self,index_name, label, source_property, embedding_property):
        # 生成嵌入向量，并添加到节点属性中
        embedding_dim = self._add_embedding(index_name, label, source_property, embedding_property)
        cypher = f"""
        ...
        """
        print(f"正在创建向量索引: {index_name}...")
        self.graph.query(cypher)
        print("向量索引创建指令已发送。")

    def _add_embedding(self, index_name,label, source_property, embedding_property):
        """
        生成嵌入向量，并添加到节点属性中，返回向量维度
        """
        # 查询所有节点的原属性值，作为模型的输入
        cypher =f"""
            Match (n:{label}) 
        ...
        for result,embedding in zip(results,embeddings):
            items.append({"id":result["id"],"embedding":embedding})

        # 智行cypher 按id查节点，写入新的嵌入向量属性
        cypher = f"""
        ...
        """
        self.graph.query(cypher,params={"batch":items})
...
if __name__ == '__main__':
    ...
    # )
    #
    # result = store.similarity_search("Apple",1)[0].page_content
    # print(result)
